...
status_t ADC16_get_value(ADC_Type *base, uint32_t *data) {
    if (base->SC1[0] & kADC16_ChannelConversionDoneFlag) {
        *data = base->R[0];
...
status_t ADC16_calibrate_RIOT(ADC_Type *base) {
    base->SC3 |= ADC_SC3_CAL_MASK;
    while ((base->SC3 & ADC_SC3_CAL_MASK) != 0) {}
    base->SC3 |= ADC_SC3_CAL_MASK;
